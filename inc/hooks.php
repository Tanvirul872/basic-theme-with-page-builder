...
if ( ! function_exists( 'understrap_add_site_info' ) ) {
	...
	function understrap_add_site_info() {
		...
		$site_info = sprintf(
			...
			esc_url( __( 'https://wordpress.org/', 'redapple' ) ),
			sprintf(
				/* translators: WordPress */
				esc_html__( 'Proudly powered by %s', 'redapple' ),
				'WordPress'
			),
			sprintf( // WPCS: XSS ok.
				/* translators: 1: Theme name, 2: Theme author */
				esc_html__( 'Theme: %1$s by %2$s.', 'redapple' ),
				$the_theme->get( 'Name' ),
				'<a href="' . esc_url( __( 'https://redapple.com', 'redapple' ) ) . '">redapple.com</a>'
			),
			sprintf( // WPCS: XSS ok.
				/* translators: Theme version */
				esc_html__( 'Version: %1$s', 'redapple' ),
