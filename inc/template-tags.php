	...
	function understrap_posted_on() {
		...
		$posted_on   = apply_filters(
			...
			sprintf(
				...
				esc_html_x( 'Posted on', 'post date', 'redapple' ),
		...
		$byline      = apply_filters(
			...
			sprintf(
				...
				$posted_on ? esc_html_x( 'by', 'post author', 'redapple' ) : esc_html_x( 'Posted by', 'post author', 'redapple' ),
...
if ( ! function_exists( 'understrap_entry_footer' ) ) {
	...
	function understrap_entry_footer() {
		...
		if ( 'post' === get_post_type() ) {
			...
			$categories_list = get_the_category_list( esc_html__( ', ', 'redapple' ) );
			if ( $categories_list && understrap_categorized_blog() ) {
				/* translators: %s: Categories of current post */
				printf( '<span class="cat-links">' . esc_html__( 'Posted in %s', 'redapple' ) . '</span>', $categories_list ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
			}
			/* translators: used between list items, there is a space after the comma */
			$tags_list = get_the_tag_list( '', esc_html__( ', ', 'redapple' ) );
			if ( $tags_list ) {
				/* translators: %s: Tags of current post */
				printf( '<span class="tags-links">' . esc_html__( 'Tagged %s', 'redapple' ) . '</span>', $tags_list ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
			}
		}
		if ( ! is_single() && ! post_password_required() && ( comments_open() || get_comments_number() ) ) {
			echo '<span class="comments-link">';
			comments_popup_link( esc_html__( 'Leave a comment', 'redapple' ), esc_html__( '1 Comment', 'redapple' ), esc_html__( '% Comments', 'redapple' ) );
			echo '</span>';
		}
...
if ( ! function_exists( 'understrap_comment_navigation' ) ) {
	...
	function understrap_comment_navigation( $nav_id ) {
		...
		<nav class="comment-navigation" id="<?php echo esc_attr( $nav_id ); ?>">
			...
			<h1 class="sr-only"><?php esc_html_e( 'Comment navigation', 'redapple' ); ?></h1>

			<?php if ( get_previous_comments_link() ) { ?>
				<div class="nav-previous">
					<?php previous_comments_link( __( '&larr; Older Comments', 'redapple' ) ); ?>
				</div>
			<?php } ?>

			<?php if ( get_next_comments_link() ) { ?>
				<div class="nav-next">
					<?php next_comments_link( __( 'Newer Comments &rarr;', 'redapple' ) ); ?>
				</div>
			<?php } ?>
...
if ( ! function_exists( 'understrap_post_nav' ) ) {
	...
	function understrap_post_nav() {
		...
		<nav class="container navigation post-navigation">
			<h2 class="sr-only"><?php esc_html_e( 'Post navigation', 'redapple' ); ?></h2>
			<div class="row nav-links justify-content-between">
				<?php
				if ( get_previous_post_link() ) {
					previous_post_link( '<span class="nav-previous">%link</span>', _x( '<i class="fa fa-angle-left"></i>&nbsp;%title', 'Previous post link', 'redapple' ) );
				}
				if ( get_next_post_link() ) {
					next_post_link( '<span class="nav-next">%link</span>', _x( '%title&nbsp;<i class="fa fa-angle-right"></i>', 'Next post link', 'redapple' ) );
				}
				?>
			</div><!-- .nav-links -->
		</nav><!-- .navigation -->
...
if ( ! function_exists( 'understrap_link_pages' ) ) {
	...
	function understrap_link_pages() {
		$args = apply_filters(
			...
			array(
				'before' => '<div class="page-links">' . esc_html__( 'Pages:', 'redapple' ),
				'after'  => '</div>',
			)
		);
		wp_link_pages( $args );
